...
columns_to_drop = ['title', 'description', 'image', "params",
                   'activation_date', 'deal_probability', 'title_norm', 'description_norm','text','param_2']
...
plt.savefig(patch +'feature_import.png')

##############################################################

y_pred = model.predict(X_te)
y_pred[y_pred < 0.0312] = 0
y_pred[y_pred > 0.9593] = 1
sub = pd.read_csv(patch + 'sample_submission.csv', nrows=nrows)
sub['deal_probability'] = y_pred
# sub['deal_probability'].clip(0.0, 1.0, inplace=True)
sub.to_csv(patch + 'text_feature+no_ridgeV2.csv', index=False)
print('done ')